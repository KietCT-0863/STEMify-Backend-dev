...
namespace Identity.Web.Middlewares
{
    ...
    ) : IMiddleware
    {
        public async Task InvokeAsync(HttpContext context, RequestDelegate next)
        {
            ...
            var cacheKey =
                $"{options.Value.DefaultCachePrefix}{userName}_{accessToken}_revoked_token";
            ...
            if (revokedToken is not null)
            {
                ...
                throw new UnAuthorizedException(
                    "Access token is revoked, User is not authorized to access this resource"
                );
            }
    ...
    {
        public static IApplicationBuilder UseRevokeAccessTokenMiddleware(
