using Classroom.Application.Common.Interfaces.Repositories;
using Classroom.Domain.Entities;
using Infrastructure.Abstractions.Persistence.EfCore;
using Sieve.Services;

namespace Classroom.Infrastructure.Persistence.Repositories
{
    public class StudentAssignmentRepository
        : EfRepositoryBase<ClassroomDbContext, StudentAssignment, int>,
            IStudentAssignmentRepository
    {
        public StudentAssignmentRepository(ClassroomDbContext context, ISieveProcessor sieveProcessor)
            : base(context, sieveProcessor) { }
    }
}
